...
class BasicQftEntanglerCZLayers(Operation):
    ...
    def compute_decomposition(weights, wires, rotation):  # pylint: disable=arguments-differ
        ...
        **Example**
        ...
        for layer in range(repeat):
            for i in range(len(wires)-nqft):
                op_list.append(rotation(weights[..., layer, i], wires=wires[i : i + 1]))
            
            for i in range (len(wires)-1):
                w = wires.subset([i, i+1], periodic_boundary=False)
                op_list.append (qml.CZ(wires=w))
