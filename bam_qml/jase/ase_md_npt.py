...
class NPT3(MolecularDynamics):
    ...
    def run(self, steps, l_pbc_wrapper=False):
        """Perform a number of time steps.
            l_pbc_wrapper : True: all particles are placed within the box.
        """
        ...
        for i in range(steps):
            if l_pbc_wrapper and (i+1)%500 == 0:
                q = np.mod(self.q, np.ones(3))
                crds = self.atoms.cell.cartesian_positions (q)
